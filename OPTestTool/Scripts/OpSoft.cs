...
{
    private const string DLL_NAME = @"./Lib/op_x64.dll";

    private int _nSize = 0;

    //const string DLL_NAME = @"./MyLib/op_x64.dll";
    ...
    { int _size = OP_AStarFindPath(_op, mapWidth, mapHeight, disable_points, beginX, beginY, endX, endY, _pStr, _nSize); if (_size > 0) { if (_nSize > 0) Marshal.FreeHGlobal(_pStr); _pStr = Marshal.AllocHGlobal(_nSize = _size); OP_AStarFindPath(_op, mapWidth, mapHeight, disable_points, beginX, beginY, endX, endY, _pStr, _nSize); } string str = Marshal.PtrToStringUni(_pStr); return str; }
    ...
    public int BindWindow(int hwnd, string display, string mouse, string keypad, int mode) =>
        OP_BindWindow(_op, hwnd, display, mouse, keypad, mode);
    ...
    public int Capture(int x1, int y1, int x2, int y2, string file_name) =>
        OP_Capture(_op, x1, y1, x2, y2, file_name);
    ...
    /// <returns>0：表示操作失败。 1：表示操作成功。</returns>
    public int ClientToScreen(int ClientToScreen, ref int x, ref int y) =>
        OP_ClientToScreen(_op, ClientToScreen, ref x, ref y);
    ...
    /// <param name="filter">表示窗口的过滤条件,可以是以下值 1:匹配窗口标题，参数 title 有效 2:匹配窗口类名，参数 class_name 有效 4:只匹配指定父窗口的第一层子窗口 8:匹配所有者窗口为 0 的窗口，即顶级窗口 16:匹配可见的窗口 32:匹配出的窗口按照窗口打开顺序依次排列</param>
    /// <returns>返回所有匹配到的窗口句柄</returns>
    public string EnumWindowByProcess(
        string process_name,
        string title,
        string class_name,
        int filter
    ...
    {
        int _size = OP_EnumWindowByProcess(
            _op,
            process_name,
            title,
            class_name,
    ...
    public int FindColor(
        ...
        out int x,
        out int y
    ) => OP_FindColor(_op, x1, y1, x2, y2, color, sim, dir, out x, out y);
    ...
    public string FindColorBlockEx(
        int x1,
        int y1,
        int x2,
        int y2,
    ...
    public int FindMultiColor(
        ...
        out int x,
        out int y
    ) => OP_FindMultiColor(_op, x1, y1, x2, y2, first_color, offset_color, sim, dir, out x, out y);
    ...
    public int FindPic(
        ...
        out int x,
        out int y
    ) => OP_FindPic(_op, x1, y1, x2, y2, files, delta_color, sim, dir, out x, out y);
    ...
    public int FindStr(
        ...
        out int retx,
        out int rety
    ) => OP_FindStr(_op, x1, y1, x2, y2, strs, color, sim, out retx, out rety);
    ...
    public int FindWindowByProcess(string process_name, string class_name, string title) =>
        OP_FindWindowByProcess(_op, process_name, class_name, title);
    ...
    public int FindWindowByProcessId(int process_id, string class_name, string title) =>
        OP_FindWindowByProcessId(_op, process_id, class_name, title);
    ...
    public int FindWindowEx(int parent, string class_name, string title) =>
        OP_FindWindowEx(_op, parent, class_name, title);
    ...
    public int GetClientRect(int hwnd, out int x1, out int y1, out int x2, out int y2) =>
        OP_GetClientRect(_op, hwnd, out x1, out y1, out x2, out y2);
    ...
    public int GetClientSize(int hwnd, out int width, out int height) =>
        OP_GetClientSize(_op, hwnd, out width, out height);
    ...
    public int GetScreenData(int x1, int y1, int x2, int y2, ref IntPtr data) =>
        OP_GetScreenData(_op, x1, y1, x2, y2, ref data);
    ...
    /// <returns>0：失败 1：成功</returns>
    public int GetScreenDataBmp(int x1, int y1, int x2, int y2, out IntPtr data, out int size) =>
        OP_GetScreenDataBmp(_op, x1, y1, x2, y2, out data, out size);
    ...
    /// <returns>0：失败 1：成功</returns>
    public void GetScreenFrameInfo(ref int frame_id, ref int time) =>
        OP_GetScreenFrameInfo(_op, ref frame_id, ref time);
    ...
    public int GetWindowRect(int hwnd, out int x1, out int y1, out int x2, out int y2) =>
        OP_GetWindowRect(_op, hwnd, out x1, out y1, out x2, out y2);
    ...
    public int InjectDll(string process_name, string dll_name) =>
        OP_InjectDll(_op, process_name, dll_name);
    ...
    public int LoadMemPic(string file_name, IntPtr data, int size) =>
        OP_LoadMemPic(_op, file_name, data, size);
    ...
    /// <returns>0: 失败 1: 成功</returns>
    public int ScreenToClient(int hwnd, ref int x, ref int y) =>
        OP_ScreenToClient(_op, hwnd, ref x, ref y);
    ...
    /// <returns>0: 失败 1: 成功</returns>
    public int SetClientSize(int hwnd, int width, int hight) =>
        OP_SetClientSize(_op, hwnd, width, hight);
    ...
    public int SetOcrEngine(string path_of_engine, string dll_name, string argv) =>
        OP_SetOcrEngine(_op, path_of_engine, dll_name, argv);
    ...
    public int SetWindowSize(int hwnd, int width, int height) =>
        OP_SetWindowSize(_op, hwnd, width, height);
    ...
    public int SetWindowTransparent(int hwnd, int trans) =>
        OP_SetWindowTransparent(_op, hwnd, trans);
    ...
    /// <returns>0: 失败 1: 成功</returns>
    public int WriteData(int hwnd, string address, string data, int size) =>
        OP_WriteData(_op, hwnd, address, data, size);
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_EnumWindow(
        IntPtr _op,
        int parent,
        string title,
        string class_name,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_EnumWindowByProcess(
        IntPtr _op,
        string process_name,
        string title,
        string class_name,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindColor(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindColorBlock(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindColorBlockEx(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindColorEx(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindLine(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindMultiColor(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindMultiColorEx(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindNearestPos(
        IntPtr _op,
        string all_pos,
        int type,
        int x,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindPic(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindPicEx(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindPicExS(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindPicS(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindStr(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindStrEx(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindWindowByProcess(
        IntPtr _op,
        string process_name,
        string class_name,
        string title
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindWindowByProcessId(
        IntPtr _op,
        int process_id,
        string class_name,
        string title
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_FindWindowEx(
        IntPtr _op,
        int parent,
        string class_name,
        string title
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetClientRect(
        IntPtr _op,
        int hwnd,
        out int x1,
        out int y1,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetCmdStr(
        IntPtr _op,
        string cmd,
        int millseconds,
        IntPtr _pStr,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetScreenData(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetScreenDataBmp(
        IntPtr _op,
        int x1,
        int y1,
        int x2,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetWindowProcessPath(
        IntPtr _op,
        int hwnd,
        IntPtr _pStr,
        int _nSize
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_GetWindowRect(
        IntPtr _op,
        int hwnd,
        out int x1,
        out int y1,
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_MatchPicName(
        IntPtr _op,
        string pic_name,
        IntPtr _pStr,
        int _nSize
