...
public partial class OpSoft: IDisposable, IComparable<OpSoft>
{
    ...
    /// <param name="display">屏幕显示模式,取值定义如下 normal:正常模式,平常我们用的前台截屏模式 normal.dxgi:dxgi 截图模式，这个速度更快，更省 CPU normal.wgc:wgc 截图模式，仅支持 win11 以上版本 gdi:gdi 模式,用于窗口采用 GDI 方式刷新时,此模式占用 CPU 较大 gdi2:gdi2 模式,此模式兼容性较强,但是速度比 gdi 模式要慢许多 dx:dx 模式,等同于 dx.d3d9 dx2:dx2 模式,用于窗口采用 dx 模式刷新 dx.d3d9:d3d9 模式,使用 d3d9 渲染 dx.d3d10:d3d10 模式,使用 d3d10 渲染 dx.d3d11:d3d11 模式,使用 d3d11 渲染 dx.d3d12:d3d12 模式,使用 d3d12 渲染 opengl:opengl 模式，使用 opengl 渲染的窗口 opengl.std:测试中 opengl.nox:opengl 模式，针对最新夜神模拟器的渲染方式，测试中... opengl.es:测试中... opengl.fi:测试中...</param>
    ...
    /// <param name="mode">模式，取值 0、1</param>
    /// <returns>0: 失败 1: 成功</returns>
    public int BindWindowEx(int display_hwnd, int input_hwnd, string display, string mouse, string keypad, int mode) => OP_BindWindowEx(_op, display_hwnd, input_hwnd, display, mouse, keypad, mode);
    /// <summary>
    ...
    /// </summary>
    /// <param name="type">键盘类型，取值： "normal" | "normal2" | "windows" | "dx"</param>
    /// <param name="delay"></param>
    /// <returns>0：失败 1：成功</returns>
    ...
    [DllImport(DLL_NAME, CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
    private static extern int OP_BindWindowEx(IntPtr _op, int display_hwnd, int input_hwnd, string display, string mouse, string keypad, int mode);
